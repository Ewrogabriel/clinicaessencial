...
-- ─────────────────────────────────────────
-- 0. Adiciona 'master' ao enum app_role se não existir
-- ─────────────────────────────────────────
DO $$
BEGIN
...
CREATE POLICY "availability_slots_all"
    ...
    USING (
        EXISTS (
            ...
            AND ur.role IN ('admin', 'gestor', 'profissional')
        )
    );
...
DROP POLICY IF EXISTS "schedule_slots_all" ON public.schedule_slots;
CREATE POLICY "schedule_slots_all"
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_availability_slots_updated_at') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_schedule_slots_updated_at') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_agendamentos_sync_slot_capacity') THEN
