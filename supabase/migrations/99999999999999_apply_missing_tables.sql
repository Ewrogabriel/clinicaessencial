-- =====================================================
-- Migration: Create missing scheduling tables
-- Date: 2026-03-17
-- Description: Ensures availability_slots and schedule_slots exist
-- =====================================================

-- 1. Create availability_slots if not exists
...
-- 2. Create schedule_slots if not exists
...
-- 3. Add column if it was somehow skipped in service layer expectations
-- We add is_available as a boolean, but it could also be a generated column
-- For simplicity and compatibility with existing code that might try to insert/update it:
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'schedule_slots' AND column_name = 'is_available'
    ) THEN
        ALTER TABLE public.schedule_slots ADD COLUMN is_available BOOLEAN DEFAULT TRUE;
    END IF;
END $$;

-- 4. Re-create book_appointment function
