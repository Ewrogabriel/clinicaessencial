...
const PacienteForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      ...
      // Get access code for new patients (it's auto-generated by migration)
      if (!isEditing && savedPatientId) {
        const { data: pacienteData, error: fetchError } = await (supabase.from("pacientes") as any)
          .select("codigo_acesso")
          .eq("id", savedPatientId)
          .single();
        
        if (fetchError || !pacienteData?.codigo_acesso) {
          console.error("[v0] Erro ao buscar código:", fetchError);
          console.error("[v0] savedPatientId:", savedPatientId);
          toast({ 
            title: "Aviso",
            description: `Paciente criado, mas houve erro ao carregar o código de acesso.`,
            variant: "destructive"
          });
        } else {
