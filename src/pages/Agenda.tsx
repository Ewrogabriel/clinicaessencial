...
const Agenda = () => {
  ...
  const handleSlotClick = (date: Date) => {
    if (!isPatient || isAdmin || isGestor || isProfissional) {
  ...
  return (
    <div className="space-y-4">
      ...
      <AgendamentoForm open={formOpen} onOpenChange={(open) => { setFormOpen(open); if (!open) setSelectedAppointmentType(undefined); }} onSuccess={refetchAgendamentos} defaultDate={selectedDate} defaultProfissionalId={selectedProfIdForForm} appointmentType={selectedAppointmentType as "sessao_avulsa" | "sessao_plano" | undefined} />
      ...
      <DayScheduleModal
        ...
        onSlotSelect={handleDayModalSlotSelect}
        defaultProfissionalId={filterProfId !== "all" ? filterProfId : undefined}
      />
    </div>
  );
};
