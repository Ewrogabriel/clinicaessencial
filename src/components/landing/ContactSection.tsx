...
export const ContactSection = ({ content }: ContactSectionProps) => {
  ...
  const whatsapp = content?.whatsapp || "5500000000000";
  const email = content?.email || "contato@essencialclinicas.com.br";
  const instagramHandle = content?.instagram || "essencialclinicas";
  const titulo = content?.titulo || "Pronto para transformar sua clínica?";
  const subtitulo = content?.subtitulo || "Preencha o formulário ou entre em contato diretamente. Nossa equipe vai te ajudar a escolher o plano ideal.";
            ...
            <CardContent className="p-0">
              <div className="grid md:grid-cols-2">
                <div className="bg-primary text-primary-foreground p-8 md:p-10 flex flex-col justify-center space-y-6">
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold font-[Plus_Jakarta_Sans]">{titulo}</h2>
                  <p className="text-primary-foreground/80">{subtitulo}</p>
                  ...
                  {sent ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="nome" placeholder="Seu nome completo" value={formData.nome} onChange={(e) => setFormData({ ...formData, nome: e.target.value })} />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="email">Email *</Label>
                        <Input id="email" type="email" placeholder="seu@email.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea id="mensagem" placeholder="Conte sobre sua clínica..." rows={3} value={formData.mensagem} onChange={(e) => setFormData({ ...formData, mensagem: e.target.value })} />
                      </div>
