...
export function CommissionRules() {
  ...
  const openCreateRule = (profId?: string) => {
    setEditingRule(null);
    setRuleForm({ profissional_id: profId || "", tipo_atendimento: "geral", percentual: "", valor_fixo: "", observacoes: "", ativo: true });
    setRuleDialogOpen(true);
  };
  ...
  if (!canManage) return <p className="text-muted-foreground text-center py-8">Sem permissão.</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      {filteredRegras.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">Crie regras para definir como cada profissional recebe comissão.</p>
          </CardContent>
        </Card>
      ) : (
        <Card>
              ...
              <TableBody>
                {filteredRegras.map((regra: any) => (
                  <TableRow key={regra.id}>
                    ...
                    <TableCell><Badge variant="outline">{getTipoLabel(regra.tipo_atendimento)}</Badge></TableCell>
                    <TableCell>{Number(regra.percentual) > 0 ? `${regra.percentual}%` : "—"}</TableCell>
                    <TableCell>{Number(regra.valor_fixo) > 0 ? `R$ ${Number(regra.valor_fixo).toFixed(2)}` : "—"}</TableCell>
                    <TableCell><Badge variant={regra.ativo ? "default" : "secondary"}>{regra.ativo ? "Ativa" : "Inativa"}</Badge></TableCell>
                    <TableCell className="max-w-[150px] truncate text-muted-foreground text-sm">{regra.observacoes || "—"}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        <Button size="icon" variant="ghost" onClick={() => openEditRule(regra)}><Pencil className="h-4 w-4" /></Button>
                        <Button size="icon" variant="ghost" onClick={() => deleteRule.mutate(regra.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
      ...
      {/* Professionals without rules */}
      {profissionais.length > 0 && (
        <Card className="border-dashed">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <Users className="h-4 w-4" /> Profissionais sem regra configurada
            </CardTitle>
          </CardHeader>
          <CardContent>
            {(() => {
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input type="number" step="0.1" value={ruleForm.percentual} onChange={(e) => setRuleForm({ ...ruleForm, percentual: e.target.value })} placeholder="Ex: 30" />
              </div>
              <div>
                <Label>Valor Fixo (R$)</Label>
                <Input type="number" step="0.01" value={ruleForm.valor_fixo} onChange={(e) => setRuleForm({ ...ruleForm, valor_fixo: e.target.value })} placeholder="Ex: 25.00" />
              </div>
            </div>
            <p className="text-xs text-muted-foreground -mt-2">Percentual é aplicado sobre o valor da sessão. Valor fixo é somado por atendimento.</p>
            <div>
              <Label>Observações</Label>
              <Textarea value={ruleForm.observacoes} onChange={(e) => setRuleForm({ ...ruleForm, observacoes: e.target.value })} placeholder="Notas sobre esta regra..." rows={2} />
            </div>
            <div className="flex items-center justify-between">
              <Label htmlFor="rule-active">Regra ativa</Label>
              <Switch id="rule-active" checked={ruleForm.ativo} onCheckedChange={(c) => setRuleForm({ ...ruleForm, ativo: c })} />
            </div>
